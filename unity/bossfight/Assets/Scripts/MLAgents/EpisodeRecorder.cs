...
public class EpisodeRecorder : MonoBehaviour
{
    ...
    public void OnEpisodeStart()
    {
        ...
        // Ensure agents are found before starting new episode
    ...
    {
        // Ensure agents are found
        if (allAgents == null || allAgents.Count == 0)
        {
            FindAllAgents();
        }
        
        // If still no agents, log warning and continue
        if (allAgents == null || allAgents.Count == 0)
        {
            Debug.LogWarning("EpisodeRecorder: No agents found! Cannot record agent classes.");
        }
        ...
        if (allAgents != null)
        {
            ...
            {
                if (agent == null) continue; // Skip null agents
    ...
    public void EndEpisode(string winCondition, float duration)
    {
        ...
        // Use provided duration, or calculate from start time if duration is 0 or invalid
        if (duration <= 0f && episodeStartTime > 0f)
        {
    ...
    void OnDisable()
    {
        // Save current episode when Unity stops playing (user presses Stop)
        if (recordEpisodes && currentEpisode != null && currentEpisode.actions != null && currentEpisode.actions.Count > 0)
        {
            // Calculate duration from start time (more accurate than frame count)
            float duration = 0f;
            if (episodeStartTime > 0f)
            {
                duration = Time.time - episodeStartTime;
            }
            else
            {
                // Fallback to frame-based calculation if start time wasn't tracked
                duration = currentFrame / 60f;
            }
            
            // Set win condition to "manual_stop" if not already set
            if (string.IsNullOrEmpty(currentEpisode.winCondition))
            {
                currentEpisode.winCondition = "manual_stop";
            }
            
            if (currentEpisode.duration <= 0f)
            {
                currentEpisode.duration = duration;
            }
            ...
            // Save the episode
            SaveEpisode(currentEpisode);
            Debug.Log($"Saved episode {currentEpisode.episode} on stop (manual stop). Duration: {duration:F2}s, Actions: {currentEpisode.actions.Count}");
        }
    }
    
    void OnApplicationQuit()
    {
        // Also save on application quit (builds)
        OnDisable();
    }
}
