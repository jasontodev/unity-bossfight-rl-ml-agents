...
public class PlayerController : MonoBehaviour
{
    ...
    void OnControllerColliderHit(ControllerColliderHit hit)
    {
        ...
        {
            if (healthSystem != null && !healthSystem.IsInVoidGrace)
            {
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Void"))
        {
            ...
            {
                // Ignore void kills during a short grace period after respawn
    ...
    void Update()
    {
        ...
        if (isGrounded && !healthSystem.IsInVoidGrace)
        {
            ...
            if (Physics.Raycast(rayStart, Vector3.down, out hit, rayDistance))
            {
                ...
                {
                    // Only set to false if we were actually in lava (to trigger burn)
                    // Don't call SetInLava(false) if we were never in lava to begin with
            ...
            {
                // Not grounded on anything - only set to false if we were actually in lava
                // This prevents false burn triggers on spawn
                if (healthSystem != null && healthSystem.GetIsInLava())
                {
                    healthSystem.SetInLava(false);
                }
    ...
    /// <summary>
    /// Reset movement state after an external teleport (e.g., EpisodeManager respawn).
    /// Clears velocity and re-syncs the CharacterController with the current transform
    /// position to avoid snapping back toward the old location.
    /// </summary>
